        ...
        <CardContent className="space-y-7 pt-6">
          <section className="space-y-4">
            ...
            {draft.buyers.map((buyer, index) => <div key={index} className="grid gap-3 rounded-xl border border-[#e5e7df] bg-[#f8f5eb] p-4 md:grid-cols-2">
              ...
              <div><Label>{documentLabel(buyer.documentType)} {(!buyer.documentType || buyer.documentType === 'dni') && '(8 dígitos)'}</Label><Input value={buyer.document} onChange={event => setBuyer(index, 'document', (!buyer.documentType || buyer.documentType === 'dni') ? event.target.value.replace(/\D/g, '').slice(0, 8) : event.target.value)} inputMode={(!buyer.documentType || buyer.documentType === 'dni') ? 'numeric' : 'text'} maxLength={(!buyer.documentType || buyer.documentType === 'dni') ? 8 : undefined} disabled={!canEdit} className={field} /></div>
